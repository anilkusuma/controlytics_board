...
@Component(value = "granulesAlarmPdfReportGenerator")
public class GranulesAlarmPdfReportGenerator extends AbstractPdfGenerator {

    @Autowired
    public GranulesAlarmPdfReportGenerator(@Value("${report_gen.storage_path}") String reportLocalStoragePath,
                                           @Qualifier(PDF_GENERATOR_TEMPLATE_BEAN) TemplateEngine templateEngine) {
        super("pdf_templates/granules-alarm-pdf-report", templateEngine, reportLocalStoragePath + "/alarm-reports" +
                "/");
    }
}
