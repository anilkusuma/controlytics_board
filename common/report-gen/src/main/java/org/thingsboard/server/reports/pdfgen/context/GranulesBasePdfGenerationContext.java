...
public class GranulesBasePdfGenerationContext extends PdfGenerationContext {
    ...
    protected Double temperatureNmtValue;
    protected Double temperatureNltValue;
    protected Double humidityNmtValue;
    protected Double humidityNltValue;
    ...
    public Context asContext() {
        ...
        context.setVariable("startTimeOnly", getFormattedTimeOnlyInIst(startTs));
        context.setVariable("endDate", getFormattedDateInIst(endTs));
        context.setVariable("endTime", getFormattedTimeInIst(endTs));
        context.setVariable("endTimeOnly", getFormattedTimeOnlyInIst(endTs));
        ...
        getSharedAttributes().forEach(attributeKvEntry -> {
            if (attributeKvEntry.getKey().equals(TEMPERATURE_NMT_ENABLED_ATTRIBUTE_KEY)) {
                ...
                final String temperatureNmtValue = format(attributeKvEntry.getValue());
                if (temperatureNmtValue != null) {
                    this.temperatureNmtValue = parse(temperatureNmtValue);
                }
                context.setVariable("temperatureNmtValue", format(attributeKvEntry.getValue()));
            } else if (attributeKvEntry.getKey().equals(TEMPERATURE_NLT_VALUE_ATTRIBUTE_KEY)) {
                final String temperatureNltValue = format(attributeKvEntry.getValue());
                if (temperatureNltValue != null) {
                    this.temperatureNltValue = parse(temperatureNltValue);
                }
                context.setVariable("temperatureNltValue",
                        format(attributeKvEntry.getValue()));
                ...
                final String humidityNltValue = format(attributeKvEntry.getValue());
                if (humidityNltValue != null) {
                    this.humidityNltValue = parse(humidityNltValue);
                }
                context.setVariable("humidityNltValue", format(attributeKvEntry.getValue()));
            } else if (attributeKvEntry.getKey().equals(HUMIDITY_NMT_VALUE_ATTRIBUTE_KEY)) {
                final String humidityNmtValue = format(attributeKvEntry.getValue());
                if (humidityNmtValue != null) {
                    this.humidityNmtValue = parse(humidityNmtValue);
                }
                context.setVariable("humidityNmtValue", format(attributeKvEntry.getValue()));
