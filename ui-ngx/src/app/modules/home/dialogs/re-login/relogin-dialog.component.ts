...
export interface ReLoginDialogComponentData {
  ...
  remarksDropDownValues?: Array<string>;
}
...
export class ReloginDialogComponent extends DialogComponent<ReloginDialogComponent, ReLoginDialogComponentResponse> implements OnInit, ErrorStateMatcher {
  ...
  ngOnInit(): void {
    this.reloginFormGroup = this.fb.group({
      username: [`${getCurrentAuthState(this.store).userDetails.email}`, [Validators.required]],
      password: ['', [Validators.required]],
    });
    if (this.data.remarksRequired && (this.data.remarksDropDownValues?.length ?? 0) === 0) {
      this.reloginFormGroup.addControl('remarks', this.fb.control('', [Validators.required, Validators.minLength(2)]));
    }

    if (this.data.remarksRequired && (this.data.remarksDropDownValues?.length ?? 0) > 0) {
      this.reloginFormGroup.addControl('remarks', this.fb.control('', [Validators.required, Validators.minLength(2)]));
      this.reloginFormGroup.addControl('remarksDropDown', this.fb.control('', [Validators.required]));
      this.reloginFormGroup.get('remarksDropDown')?.valueChanges.subscribe((value) => {
        this.reloginFormGroup.get('remarks')?.setValue(this.data.remarksDropDownValues[value]);
      });
      this.reloginFormGroup.get('remarks')?.setValue(this.data.remarksDropDownValues[0]);
      this.reloginFormGroup.get('remarksDropDown')?.setValue(0);
    }
  ...
  login() {
    ...
    if (this.reloginFormGroup.valid) {
      ...
      if ((this.data.userNameInputRequired ?? false)
        && (this.reloginFormGroup.get('username').value !== getCurrentAuthState(this.store).userDetails.email)) {
      ...
      this.authService.relogin({
        username: this.data.userNameInputRequired ? this.reloginFormGroup.get('username').value : getCurrentAuthState(this.store).userDetails.email,
