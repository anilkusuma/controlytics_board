...
export class DevicesTableConfigResolver implements Resolve<EntityTableConfig<DeviceInfo>> {
  ...
  configureCellActions(deviceScope: string): Array<CellActionDescriptor<DeviceInfo>> {
    ...
    if (deviceScope === 'tenant' && this.userRole === UserRole.MAINTENANCE) {
      actions.push({
    ...
    if (deviceScope === 'tenant' && this.userRole === UserRole.CONTROLYTICS_ADMIN) {
      actions.push(
        {
    ...
    if (deviceScope === 'customer'  && this.userRole === UserRole.CONTROLYTICS_ADMIN) {
      actions.push(
        {
          name: this.translate.instant('device.unassign-from-customer'),
          icon: 'assignment_return',
    ...
    if ((deviceScope === 'customer_user' || deviceScope === 'edge_customer_user')
      && this.userRole === UserRole.CONTROLYTICS_ADMIN) {
      actions.push(
        {
    ...
    if (deviceScope === 'edge' && this.userRole === UserRole.CONTROLYTICS_ADMIN) {
      actions.push(
        {
